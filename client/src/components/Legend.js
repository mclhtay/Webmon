...
const Legend = ({
  ...
  const legendMonSprites = legendMons.map((mon, index) => (
  ...
  const handleSecondaryViewport = (e) => {
    handleViewportChange("legend", e.currentTarget.name);
    if (e.currentTarget.name === "battle") {
      const randomOpponent =
        legendMons[Math.floor(Math.random() * legendMons.length)];
      const randomBPMultiplier = Math.floor(Math.random() * 3) + 36;
      const randomBP =
        Math.floor(
          Math.random() *
  ...
  const handleCatch = () => {
    ...
    handleViewportChange("legend", "postcatch");
  };

  return (
    <div className={viewport === "legend" ? "modal-frame come-in" : "blind"}>
      <div className="modal-content legend-modal">
        <div className="modal-foreground">
          <h3 className="modal-title styled-font">Legendary</h3>
          ...
          <div className={secondary === "main" ? "come-in" : "blind"}>
            ...
            <div className="row">{legendMonSprites}</div>
            <h4>Item drops: </h4>
