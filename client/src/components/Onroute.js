...
const Onroute = ({
  ...
  const handleSecondaryViewport = (e) => {
    handleViewportChange("onroute", e.currentTarget.name);
    if (e.currentTarget.name === "battle") {
      ...
      if (bp > obp || Math.floor(Math.random() * (obp - bp)) === 0) {
        setResult({
          ...reward,
          result: "victory",
          rewardCoins: Math.floor(Math.random() * 41) + 10,
          rewardCandy: Math.floor(Math.random() * 20) === 0 ? 1 : 0,
          rewardExp: bp > obp ? 10 : 10 + (obp - bp),
        });
      } else {
        setResult({
          ...reward,
  ...
  const handleCatch = () => {
    ...
    handleViewportChange("onroute", "postcatch");
  };
  return (
    <div className={viewport === "onroute" ? "modal-frame  come-in" : "blind"}>
      <div className="modal-content onroute-modal">
        <div className="modal-foreground">
          <h3 className="modal-title styled-font">On Route</h3>
          <span className="close-modal" onClick={handleViewport} name="exit">
            &#10008;
          </span>
          <div className={secondary === "main" ? "come-in" : "blind"}>
            <p>
              You can advance to the "Gym" difficulty when your highest BP
              reaches 200
            </p>
            <p>
            ...
            <div className="row">{onrouteMonsSprite}</div>
            <h4>Item drops: </h4>
            <i className="fas fa-coins" />: 10 - 50
            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            <i className="fas fa-candy-cane" />: 5%
            <div>
              <button
                type="button"
                name="battle"
          ...
          <div className={secondary === "postcatch" ? "come-in" : "blind"}>
            ...
            >
              {pokemons.length === bagSize ? (
                ...
                <span>
                  Catch another &nbsp;&nbsp; <i className="fas fa-coins" /> 200
                </span>
              )}
            </button>
            <button
